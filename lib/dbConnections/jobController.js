...
class JobController {
    ...
    getAllJobs(){
        return (async () => {
            const client = await pool.connect();
            try {
                const sQuery = await client.query(jobQuery.get_all_rows);
                if (sQuery.rowCount === 1) {
                    result.message = `${responseText.record_found}`;
        ...
        return (async () => {
            ...
            try {
                ...
                if (sQuery.rowCount === 0) {
                    ...
                    const cQuery = await client.query(jobQuery.create_row, [name, description, frequency, startdate, script,repeat]);
                    id = cQuery.rows[0].id;
        ...
        return (async () => {
            ...
            try {
                ...
                if (sQuery.rowCount === 1) {
                    ...
                    result.message = responseText.record_modified;
                    result.data = mQuery.rows[0];
                    result.status = status.success;
                    return result;
                } else {
                    result.message = responseText.record_not_available;
        ...
        return (async () => {
            ...
            try {
                ...
                if (sQuery.rowCount === 1) {
                    ...
                    result.message = responseText.record_deleted;
                    result.data = id;
                    result.status = status.success;
                    return result;
                } else {
